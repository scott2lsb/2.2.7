...
public class GuanzhuFriendParser extends JSONParser {
	private Context context;

	public GuanzhuFriendParser() {

	}

	public GuanzhuFriendParser(Context context) {
		...
		try {
			...
			if (errCode == JSONMessageType.SERVER_CODE_OK) {
				...
				for (int i = 0; i < users.length(); ++i) {
					...
					u.level = user.getString("userLevel");
					List<EventData> le = new ArrayList<EventData>();
					JSONArray events = user.optJSONArray("event");
					if (events != null) {
