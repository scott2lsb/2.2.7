...
public class UserMailActivity extends Activity implements OnClickListener,
		OnItemClickListener, NetServerListener, NetConnectionListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		connectBg.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				hidepb();
				server.removeListener(UserMailActivity.this);
				server.cancel();
			}
		});
	...
	public void onClick(View v) {
		...
		case R.id.privatem_input_send:
			...
			if (input.getText().toString().trim().length() <= 0) {
				...
				server.service(new MailSendRequest(otherUserId),
						new MailSendParser(uo));
			}
			break;
					...
					public void onClick(DialogInterface dialog, int item) {
						...
						if (item == 0) {
							...
							UserNow.current().picPath = camerFile
									.getAbsolutePath();
							theSmall = JSONMessageType.USER_PIC_SDCARD_FOLDER
									+ fileName;
	...
	private void startVoice() {
		...
		String str = getResources().getString(R.string.voicewarning);
	...
	// UserMailActivity.this, theSmall);
	...
	// Toast.makeText(UserMailActivity.this, "请选择图片文件！",
	...
	// // Intent intent = new Intent(UserMailActivity.this,
	// // VoiceResultActivity.class);
	// // intent.putStringArrayListExtra("voice_result", matches);
	// // startActivityForResult(intent,
	// // VOICE_RECOGNITION_RESULT_REQUEST);
	// break;
	// case VOICE_RECOGNITION_RESULT_REQUEST:
	...
	@Override
	public void onBegin() {
		serverHandler.sendEmptyMessage(JSONMessageType.NET_BEGIN);
	}
	...
	public void onResponse(String errorMessage) {
		...
		msg.obj = errorMessage == null ? null : errorMessage;
		serverHandler.sendMessage(msg);
	...
	public void onNetEnd(String msg, String method) {
		...
		if (method.equals("mailDetail")) {
			...
			String s = new ResultParser().parse(msg);
			if (s != null && s.equals(""))
				updateList();
			else
				Toast.makeText(this, s, Toast.LENGTH_SHORT).show();
